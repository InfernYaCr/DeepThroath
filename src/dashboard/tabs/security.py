...
from src.reports.pdf_export import export_pdf, render_html_report, render_markdown_report
...
def render_security(
    ...
    with tab2:
        _render_owasp_tab(df)

    with tab3:
        st.plotly_chart(passrate_trend(history_df), width="stretch")

    with tab4:
        _render_comparison_tab(scan_files)

    with tab5:
        render_logs_table(df)

    st.divider()
    _render_export(df, history_df)


def _build_cat_map(df: pd.DataFrame) -> list[dict]:
    cat_map: dict = {}
...
def _render_overview_tab(df: pd.DataFrame) -> None:
    ...
    if failed_cats:
        ...
        for c in failed_cats[:5]:
            cat_full = next(
                (get_owasp_category(v) for v in df["vulnerability"] if get_owasp_category(v).id == c["id"]),
                None,
            )
...
def _render_owasp_tab(df: pd.DataFrame) -> None:
...
def _render_comparison_tab(scan_files: list) -> None:
    ...
    for v in all_vulns:
        ...
        if asr_a is not None and asr_b is not None:
            delta = asr_b - asr_a
            trend = "✅ улучшилось" if delta < -0.01 else ("🔴 ухудшилось" if delta > 0.01 else "➡️ без изменений")
        elif asr_a is None:
            delta = None
            trend = "🆕 новый тест"
        else:
...
def _render_export(df: pd.DataFrame, history_df: pd.DataFrame) -> None:
